...
const loginHandle = () => {
  ...
  if (logInfo.value.account === '' || logInfo.value.password === '') {
    messagePop('Account or password is empty');
    return;
  }
  visitorLogin(logInfo.value.account, logInfo.value.password).then((res) => {
...
const registerHandle = () => {
  ...
  if (logInfo.value.password !== logInfo.value.verify) {
    messagePop('Password is not the same');
    return;
  }
